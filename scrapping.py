...
def main():
    ...
    with sync_playwright() as p:
        ...
        for idx, url in enumerate(product_links[:2]):  # Batasi 10 produk dulu
    ...
    if all_data:
        ...
        # Simpan ke CSV
        with open("dunia-anak-ceria_tokopedia_full_data.csv", "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=all_keys)
            writer.writeheader()
            writer.writerows(all_data)
        print("✅ Data berhasil disimpan ke dunia-anak-ceria_tokopedia_full_data.csv")
    else:
